...
rota.get('/consultarNotaTotal', (req, res) => {
    ...
    const dados = lerDados();
    const dadosFiltrados = dados.grades.filter(grade => grade.student === student && grade.subject === subject);

    const nota = dadosFiltrados.reduce((total, atual) => (total = total + atual.value), 0);
...
rota.get('/consultarMediaDasGrades', (req, res) => {
    ...
    const dados = lerDados();
    const dadosFiltrados = dados.grades.filter(x => x.subject === subject && x.type === type);
    const nota = dadosFiltrados.reduce((total, atual) => (total = total + atual.value), 0);
...
function gravaDados(dados) {
    ...
    fs.writeFileSync(path.join(__dirname, '/dados', 'grades.json'), JSON.stringify(dados), 'utf8');
};
...
function criarNovaGrade(dados, grade) {
    ...
    fs.writeFileSync(path.join(__dirname, '/dados', 'grades.json'), JSON.stringify(novaGrade), 'utf8');
